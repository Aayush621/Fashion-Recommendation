...
import numpy as np
import os
from qdrant_client import QdrantClient, models # For Qdrant
...
CLIP_MODEL: Optional[Any] = None # Using Any for type hint flexibility with clip.load
...
class TextSearchQuery(BaseModel):
    ...
    top_n: Optional[int] = Field(TOP_N_DEFAULT, gt=0, le=20, description="Number of results to return.")
...
async def shutdown_event():
    # Placeholder for actual client closing if needed in future versions of qdrant_client
...
def format_qdrant_results(hits: List[models.ScoredPoint]) -> List[ProductSuggestion]:
    ...
    for i, hit in enumerate(hits):
        ...
        if product_id and S3_BUCKET_BASE_URL:
            # Construct the full, public URL to the image on S3
            # This simple join avoids double slashes
...
async def api_image_to_image_search(
    ...
    try:
        ...
        # Fetch one extra to handle potential self-match
        search_hits_raw = QDRANT_CLIENT.search(
            collection_name=IMAGE_COLLECTION_NAME,
            query_vector=query_vector,
            limit=top_n + 1, # Fetch one extra for self-exclusion
            with_payload=True
        )

        final_hits = search_hits_raw[:top_n] # Take the top_n actual results
        ...
        await query_image.close() # Important to close the uploaded file
